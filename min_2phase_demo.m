function [opt mat] = min_2phase_demo( A,B,C)
...
disp('The intial tebula is ready: ')
X
keyboard
...
while z_v > R% this is the required condition for max
    ...
    Min_R = X(2:xm,1)./X(2:xm,z_i+1);%finding min ratio
     ...
     fprintf('Maximum value in indication row : %.4f \n',z_v)
...
if abs(X(1,1)) <= R
    disp(' The min value of r is 0 & and hence we can proceed for Phase-2');
else
    disp(' Min value of r is not 0 and hence no feasible solution');
    return
end
...
[ jm jn ] = size(J);  %J matrix is defined above as J= find(-1 == X(1,:));
...
% Here after we can proceed to get solution of the problem using simplex
% method
[xm xn] = size(X);
[z_v z_i ] = max(X( 1,2:xn));
